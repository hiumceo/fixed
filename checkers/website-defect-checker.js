// ============================================================
// WEBSITE DEFECT CHECKER v1124
// ============================================================
// Composed from the exact supplied/proven:
...
//   page - video.txt
//
// Current page only. No report is created for a category with
...
`);
  ...
  // ------------------------------------------------------------
    ...
    return { items, results, report: built.report, broken: built.broken, summary: built.summary };
  }
  ...
  function downloadCategoryReport(report, category, count) {
    ...
    /*
     * Keep the exact generated TXT available to the Ultimate API.
     * The existing browser download is still performed below.
     */
    const reportStore =
      window.__V1124_ULTIMATE_REPORTS ||
      {};

    reportStore[category.toUpperCase()] = {
      filename,
      content: report,
    };
    ...
    console.log(`Report saved as: ${filename} (${count} confirmed ${category} defect(s))`);
  }

  function classifyReport(report, category, count) {
    return [
      `WEBSITE DEFECT CHECKER v1124 — ${category.toUpperCase()} REPORT`,
      "========================================",
      "",
  ...
  /*
   * Ultimate live-result data.
   * This exposes the same summary values already calculated
   * by each individual engine. It does not change how defects
   * are classified or when reports are generated.
   */
  console.log(
    `ULTIMATE LINK SUMMARY: CHECKED ${linkScan.summary.total} | MANUAL ${linkScan.summary.manual ?? 0} | DEFECTS ${linkScan.broken.length}`
  );

  console.log(
    `ULTIMATE IMAGE SUMMARY: CHECKED ${imageScan.summary.total} | MANUAL ${imageScan.summary.manual} | DEFECTS ${imageScan.broken.length}`
  );

  console.log(
    `ULTIMATE VIDEO SUMMARY: CHECKED ${videoScan.summary.total} | MANUAL ${videoScan.summary.manual} | DEFECTS ${videoScan.broken.length}`
  );
  ...
  if (counts.link > 0) {
    ...
    console.log("No confirmed broken links — no link report downloaded.");
  }
  ...
  if (counts.image > 0) {
    ...
    console.log("No confirmed broken images — no image report downloaded.");
  }
  ...
  if (counts.video > 0) {
    ...
    console.log("No confirmed broken videos — no video report downloaded.");
  }
