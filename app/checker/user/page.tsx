...
function CheckerUserContent() {
  const [account, setAccount] = useState<AccountState>(() => normalizeAccountState(null));
  const [authUser, setAuthUser] = useState<AuthUser | null>(null);
  type EngineStats = { runs: number; checked: number; manual: number; defects: number; successful: number; failed: number };
  const emptyEngine = (): EngineStats => ({ runs: 0, checked: 0, manual: 0, defects: 0, successful: 0, failed: 0 });
  const [checkerStats, setCheckerStats] = useState<Record<"LINK" | "IMAGE" | "VIDEO" | "ULTIMATE_LINK" | "ULTIMATE_IMAGE" | "ULTIMATE_VIDEO", EngineStats>>({
    LINK: emptyEngine(), IMAGE: emptyEngine(), VIDEO: emptyEngine(),
    ULTIMATE_LINK: emptyEngine(), ULTIMATE_IMAGE: emptyEngine(), ULTIMATE_VIDEO: emptyEngine(),
  });
  ...
  useEffect(() => {
    fetch("/api/account", { cache: "no-store" }).then(async r => r.ok ? r.json() : null).then(data => {
      ...
      if (data?.checkerStats) {
        setCheckerStats(() => {
          const read = (key: string): EngineStats => ({
            runs: Number(data.checkerStats[key]?.runs || 0), checked: Number(data.checkerStats[key]?.checked || 0), manual: Number(data.checkerStats[key]?.manual || 0), defects: Number(data.checkerStats[key]?.defects || 0), successful: Number(data.checkerStats[key]?.successful || 0), failed: Number(data.checkerStats[key]?.failed || 0),
          });
          return { LINK: read("LINK"), IMAGE: read("IMAGE"), VIDEO: read("VIDEO"), ULTIMATE_LINK: read("ULTIMATE_LINK"), ULTIMATE_IMAGE: read("ULTIMATE_IMAGE"), ULTIMATE_VIDEO: read("ULTIMATE_VIDEO") };
        });
      }
  ...
  const engines = ["LINK", "IMAGE", "VIDEO", "ULTIMATE_LINK", "ULTIMATE_IMAGE", "ULTIMATE_VIDEO"] as const;
  const totalCheckerRuns = engines.reduce((sum, key) => sum + checkerStats[key].runs, 0);
  const successfulRuns = engines.reduce((sum, key) => sum + checkerStats[key].successful, 0);
  const failedRuns = engines.reduce((sum, key) => sum + checkerStats[key].failed, 0);
  const totalChecks = engines.reduce((sum, key) => sum + checkerStats[key].checked, 0);
  const totalDefects = engines.reduce((sum, key) => sum + checkerStats[key].defects, 0);
    ...
    <main className="min-h-screen overflow-hidden bg-[#020805] text-white">
      ...
      <div className="relative mx-auto min-h-screen max-w-6xl px-6 py-10 sm:px-10">
        ...
        <header className="mx-auto max-w-4xl text-center">
          <img src="/bug-checker-logo.png" alt="v1124 Bug Checker" className="mx-auto w-full max-w-sm object-contain" />
            ...
            {statsTab === "overview" ? (
              <>
                ...
                <div className="mt-3 grid grid-cols-1 gap-2 lg:grid-cols-2">
                  {([
                    ["TOTAL CHECKS", [["LINK", checkerStats.LINK.checked], ["IMAGE", checkerStats.IMAGE.checked], ["VIDEO", checkerStats.VIDEO.checked], ["ULTIMATE LINK", checkerStats.ULTIMATE_LINK.checked], ["ULTIMATE IMAGE", checkerStats.ULTIMATE_IMAGE.checked], ["ULTIMATE VIDEO", checkerStats.ULTIMATE_VIDEO.checked]]],
                    ["TOTAL DEFECTS", [["LINK", checkerStats.LINK.defects], ["IMAGE", checkerStats.IMAGE.defects], ["VIDEO", checkerStats.VIDEO.defects], ["ULTIMATE LINK", checkerStats.ULTIMATE_LINK.defects], ["ULTIMATE IMAGE", checkerStats.ULTIMATE_IMAGE.defects], ["ULTIMATE VIDEO", checkerStats.ULTIMATE_VIDEO.defects]]],
              ...
              <div className="mt-4">
                {(() => {
                  const directKey = statsTab === "link" ? "LINK" : statsTab === "image" ? "IMAGE" : "VIDEO";
                  const renderRows = (stat: EngineStats) => [["TOTAL RUNS", stat.runs], ["TOTAL CHECKS", stat.checked], ["MANUAL VERIFICATION", stat.manual], ["CONFIRMED DEFECTS", stat.defects], ["SUCCESSFUL RUNS", stat.successful], ["FAILED RUNS", stat.failed]] as Array<[string, number]>;
                  if (statsTab === "ultimate") return <div className="grid grid-cols-1 gap-3">{([["ULTIMATE LINK", checkerStats.ULTIMATE_LINK], ["ULTIMATE IMAGE", checkerStats.ULTIMATE_IMAGE], ["ULTIMATE VIDEO", checkerStats.ULTIMATE_VIDEO]] as Array<[string, EngineStats]>).map(([title, stat]) => <div key={title} className="rounded-xl border border-green-500/40 bg-green-500/[0.05] p-3"><p className="text-body font-black tracking-[0.11em] text-green-400">{title}</p><div className="mt-2 grid grid-cols-2 gap-2">{renderRows(stat).map(([label, value]) => <div key={label} className="rounded-lg border border-green-500/40 bg-green-500/[0.055] p-2"><div className="flex items-center justify-between gap-2"><p className="text-body font-black tracking-[0.08em] text-green-400">{label}</p><p className="text-primary font-black text-white">{value}</p></div></div>)}</div></div>)}</div>;
                  return <div className="grid grid-cols-2 gap-2">{renderRows(checkerStats[directKey]).map(([label, value]) => <div key={label} className="rounded-xl border border-green-500/40 bg-green-500/[0.05] p-3"><div className="flex items-center justify-between gap-2"><p className="text-body font-black tracking-[0.08em] text-green-400">{label}</p><p className="text-primary font-black text-white">{value}</p></div></div>)}</div>;
                })()}
              </div>
            )}
          </div>
...
export default function CheckerUserPage() { return <AuthGate returnTo="/checker/user"><CheckerUserContent /></AuthGate>; }
